...
function Object(firstName, lastName, tech){
...
const cvFormatter = (cvInput) => {
    // write your code here
    let newArr=[];
    for(let i=0 ; i < cvInput.length ;i++ ){
        if(cvInput[i].yearsOfExperience > 1){
            let newCv=new Object(cvInput[i].firstName,cvInput[i].lastName,cvInput[i].tech);
            newArr.push(newCv);
        }
    };
    return (newArr);
